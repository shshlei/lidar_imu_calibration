...
{
LidarOdometry::LidarOdometry(double ndt_resolution, const std::string & lo_trajectory_filename, bool downsample_for_mapping)
...
{
  auto ndt_omp = pclomp::NormalDistributionsTransform<VPoint, VPoint>::Ptr(new pclomp::NormalDistributionsTransform<VPoint, VPoint>());
...
bool LidarOdometry::checkKeyScan(const Odom & odom)
{
  ...
  Eigen::Vector3d ypr = R2ypr(rotation);
...
void LidarOdometry::setTargetMap(const VPointCloud::Ptr map_cloud_in)
{
  map_cloud_->clear();
