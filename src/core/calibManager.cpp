...
#include "lidar_imu_calibration/utils/color.h"

#include <rclcpp/rclcpp.hpp>

namespace lidar_imu_calibration 
{
lidar_imu_calibration::CalibManager::CalibManager(const CalibManagerOptions & params_)
{
  /// Startup  message
  ...
  calib_extrinsic_csv.open(params.lidar_inertial_calib_extrinsic_filename);
  calib_dt_csv.open(params.lidar_inertial_calib_dt_filename);
  ...
  LOdom = std::make_shared<LidarOdometry>(params.ndt_resolution, params.lidar_odometry_trajectory_filename, params.downSampleForMapping);
}

void lidar_imu_calibration::CalibManager::feed_measurement_imu(double timestamp, const Eigen::Vector3d & wm, const Eigen::Vector3d & am)
{
...
void lidar_imu_calibration::CalibManager::feed_measurement_lidar(double timestamp, const TPointCloud ::Ptr cloud_raw)
{
  if (!is_initialized_linkalibr) {
  ...
  if (params.do_undistortion) {
    ...
    do_undistortion(timestamp, cloud_raw, cloud_undistorted, T_ndt_predict);
  ...
  if (state->_clones_IMU.size() == 1) {
    ...
    Eigen::Matrix3d I1_R_G = lidar_imu_calibration::quat_2_Rot(q_GtoI1);
...
void lidar_imu_calibration::CalibManager::do_undistortion(double timestamp, const TPointCloud::Ptr & scan_raw, TPointCloud::Ptr & scan_out, Eigen::Matrix4d & T_ndt_predict)
{
  ...
  propagator->fast_state_propagate(state, timestamp + (double)*min_it, imu_state_plus_start);
  ...
  Eigen::Matrix<double, 3, 3> start_R_G = lidar_imu_calibration::quat_2_Rot(quat_start);

  Eigen::Matrix<double, 13, 1> imu_state_plus_end;
  propagator->fast_state_propagate(state, timestamp + (double)*max_it, imu_state_plus_end);
  ...
  Eigen::Matrix<double, 3, 3> end_R_G = lidar_imu_calibration::quat_2_Rot(quat_end);
...
Eigen::Vector3d lidar_imu_calibration::CalibManager::deskewPoint(const Eigen::Matrix<double, 13, 1> & start_point_state, const Eigen::Matrix<double, 13, 1> & current_point_state,
  const Eigen::Vector3d & skewedPoint, const Eigen::Matrix3d & I_R_L, const Eigen::Vector3d & I_t_L)
{
  ...
  Eigen::Matrix<double, 3, 3> Istart_R_G = lidar_imu_calibration::quat_2_Rot(Istart_q_G);
  ...
  Eigen::Matrix<double, 3, 3> Icurr_R_G = lidar_imu_calibration::quat_2_Rot(Icurr_q_G);
...
bool lidar_imu_calibration::CalibManager::try_to_initialize()
{
...
void lidar_imu_calibration::CalibManager::do_propagate_update(double timestamp)
{
  ...
  if (state->_clones_IMU.size() < 2) {
    printf(YELLOW "[lidar_imu_calibration::CalibManager::do_propagate_update] state->_clones_IMU.size() must be > 2\n");
    return;
  }
...
void lidar_imu_calibration::CalibManager::printState()
{
  Pose * calib = state->_calib_LIDARtoIMU;

  /// 1
  trajfile_csv << rclcpp::Time(state->_timestamp).seconds() << ", "
               << state->_imu->quat().x() << ", " << state->_imu->quat().y() << ", " << state->_imu->quat().z() << ", " << state->_imu->quat().w() << ", "
               << state->_imu->pos().x() << ", " << state->_imu->pos().y() << ", " << state->_imu->pos().z() << ", "
               << state->_imu->vel().x() << ", " << state->_imu->vel().y() << ", " << state->_imu->vel().z() << std::endl;
