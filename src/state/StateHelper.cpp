...
#include "lidar_imu_calibration/state/StateHelper.h"
...
{
void StateHelper::EKFPropagation(State * state,  /// Pointer to the state
  ...
  const Eigen::MatrixXd & Q)                     /// Additive state propagation noise matrix
{
  ...
  for (size_t i = 0; i < order_OLD.size(); i++) {
    ...
    Cov_PhiT.noalias() += state->_Cov.block(0, var->id(), state->_Cov.rows(), var->size()) * Phi.block(0, Phi_id[i], Phi.rows(), var->size()).transpose();
  }
...
void StateHelper::EKFUpdate(State * state,  /// Pointer to the state
  ...
  const Eigen::MatrixXd & R)                /// updating measurment covariance
{
  ...
  //    /// Analysing the update step
  //    std::cout << "H [rows, cols]: [" << H.rows() << ", " << H.cols() << "]" << std::endl;
  //    std::cout << "res size: " << res.size() << std::endl;
  //    /// Analysing state variables
  //    std::cout << "State Variables" << std::endl;
  //    std::cout << "Id" << "\t" << "Size" << std::endl;
  //    for (Type *var : state->_variables) {
  //        std::cout << var->id() << "\t" << var->size() << std::endl;
  //    }
  //    std::cout << std::endl;
  //    /// Analysing measurement variables
  //    std::cout << "Measurement Variables" << std::endl;
  //    std::cout << "Id" << "\t" << "Size" << std::endl;
  //    for (Type *meas_var : H_order) {
  //        std::cout << meas_var->id() << "\t" << meas_var->size() << std::endl;
  //    }
  //    std::cout << std::endl;
  ///==========================================================
  ///==========================================================
  ...
  Eigen::MatrixXd K = M_a * Sinv.selfadjointView<Eigen::Upper>();  /// K =  P H_{big}^{T}(H * P_small * H^{T} + R)^{-1}
                                                                   /// H_{big} : Basically, the blocks corresponding to X_I,
                                                                   /// td, X_c will be zero
  /// Update Covariance
...
bool StateHelper::initialize(State * state,
  Type * new_variable,
...
void StateHelper::initialize_invertible(State * state,
  Type * new_variable,
  const std::vector<Type *> & H_order,
...
void StateHelper::augment_clone(State * state, Eigen::Matrix<double, 3, 1> last_w)
{
  ...
  Pose * pose = dynamic_cast<Pose *>(posetemp);
  ...
  if (state->_options.do_calib_lidar_imu_timeoffset) {
    ...
    state->_Cov.block(0, pose->id(), state->_Cov.rows(), 6) +=
      state->_Cov.block(0, state->_calib_dt_LIDARtoIMU->id(), state->_Cov.rows(), 1) * dnc_dt.transpose();
    state->_Cov.block(pose->id(), 0, 6, state->_Cov.rows()) +=
      dnc_dt * state->_Cov.block(state->_calib_dt_LIDARtoIMU->id(), 0, 1, state->_Cov.rows());
  }
}
}
