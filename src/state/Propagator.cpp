...
void Propagator::fast_state_propagate(const Eigen::Matrix<double, 10, 1> & state,
  const Eigen::Matrix<double, 3, 1> & bg,
  const Eigen::Matrix<double, 3, 1> & ba,
  ...
  double time_1)
{
  ...
  /// First lets construct an IMU vector of measurements we need
  std::vector<IMUDATA> prop_data = Propagator::select_IMU_readings(imu_data, time_0, time_1);
  ...
  if (prop_data.size() > 1) {
    for (size_t i = 0; i < prop_data.size() - 1; i++) {
      ...
      Eigen::Vector4d quat_GtoI = state_plus.block(0, 0, 4, 1);
      Eigen::Vector3d pos_IinG = state_plus.block(4, 0, 3, 1);
      Eigen::Vector3d vel_IinG = state_plus.block(7, 0, 3, 1);
      ...
      Eigen::Matrix<double, 3, 3> R_GtoI = lidar_imu_calibration::quat_2_Rot(quat_GtoI);
      ...
      state_plus.block(0, 0, 4, 1) = new_q;
      state_plus.block(4, 0, 3, 1) = new_p;
      state_plus.block(7, 0, 3, 1) = new_v;
    }
  }
}
