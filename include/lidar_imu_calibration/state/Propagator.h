...
#ifndef LIDAR_IMU_CALIBRATION_PROPAGATOR_H
#define LIDAR_IMU_CALIBRATION_PROPAGATOR_H
...
public:
  ...
  /// Default constructor
  Propagator(const NoiseManager & noises, const Eigen::Vector3d & gravity)
  {
    _noises = noises;
    _gravity = gravity;
  ...
  void feed_imu(double timestamp,  /// timestamp of any imu reading
    const Eigen::Vector3d & wm,            /// Gyro angular reading
    const Eigen::Vector3d & am)            /// Accelerometer reading
  {
  ...
  void fast_state_propagate(const Eigen::Matrix<double, 10, 1> & state,
  ...
  std::vector<IMUDATA> get_imu_data() const
  {
    return imu_data;
  }
...
#endif  // LIDAR_IMU_CALIBRATION_PROPAGATOR_H
