...
#ifndef LIDAR_IMU_CALIBRATION_QUAT_H
#define LIDAR_IMU_CALIBRATION_QUAT_H
...
public:
  ...
  void update(const Eigen::VectorXd dx) override
  {
    ...
    dq << .5 * dx, 1.0;
    dq = lidar_imu_calibration::quatnorm(dq);

    // Update estimate and recompute R
    set_value(lidar_imu_calibration::quat_multiply(dq, _value));
  }
  ...
  void set_value(const Eigen::MatrixXd new_value) override
  {
    ...
    _R = lidar_imu_calibration::quat_2_Rot(new_value);
  }
  ...
  void set_fe(const Eigen::MatrixXd new_value) override
  {
    ...
    // compute associated rotation
    _Rfe = lidar_imu_calibration::quat_2_Rot(new_value);
  }
...
#endif  // LIDAR_IMU_CALIBRATION_QUAT_H
